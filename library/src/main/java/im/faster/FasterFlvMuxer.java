...
public class FasterFlvMuxer {
    ...
    private FasterAllocator mVideoAllocator = new FasterAllocator(VIDEO_ALLOC_SIZE);
    private FasterAllocator mAudioAllocator = new FasterAllocator(AUDIO_ALLOC_SIZE);
    private ConcurrentLinkedQueue<SrsFlvFrame> mFlvTagCache = new ConcurrentLinkedQueue<>();

    private static final int VIDEO_TRACK = 100;
    private static final int AUDIO_TRACK = 101;
    private static final String TAG = "FasterFlvMuxer";

    /**
     * constructor.
     * @param handler the rtmp event handler.
     */
    public FasterFlvMuxer(RtmpHandler handler) {
    ...
    public int addTrack(MediaFormat format) {
        if (format.getString(MediaFormat.KEY_MIME).contentEquals(FasterEncoder.VCODEC)) {
    ...
    public void stop() {
        ...
        Log.i(TAG, "FasterFlvMuxer closed");
    ...
    private class SrsFlvFrame {
        // the tag bytes.
        public FasterAllocator.Allocation flvTag;
    ...
    private class SrsRawH264Stream {
        private final static String TAG = "FasterFlvMuxer";
        ...
        public FasterAllocator.Allocation muxFlvTag(ArrayList<SrsFlvFrameBytes> frames, int frame_type,
            ...
            FasterAllocator.Allocation allocation = mVideoAllocator.allocate(size);
    ...
    private class SrsFlv {
        ...
        private FasterAllocator.Allocation audio_tag;
        private FasterAllocator.Allocation video_tag;
...
//                    writeH264SpsPps(dts, pts);
        ...
        private void writeRtmpPacket(int type, int dts, int frame_type, int avc_aac_type, FasterAllocator.Allocation tag) {
