 ...
 */
public class FasterEncoder {
    private static final String TAG = "FasterEncoder";
    ...
    private FasterEncodeHandler mHandler;

    private FasterFlvMuxer flvMuxer;
    private FasterMp4Muxer mp4Muxer;
    ...
    public FasterEncoder(FasterEncodeHandler handler) {
        mHandler = handler;
        mVideoColorFormat = chooseVideoEncoder();
    }

    public void setFlvMuxer(FasterFlvMuxer flvMuxer) {
        this.flvMuxer = flvMuxer;
    }

    public void setMp4Muxer(FasterMp4Muxer mp4Muxer) {
    ...
    public AudioRecord chooseAudioRecord() {
        AudioRecord mic = new AudioRecord(MediaRecorder.AudioSource.VOICE_COMMUNICATION, FasterEncoder.ASAMPLERATE,
            AudioFormat.CHANNEL_IN_STEREO, AudioFormat.ENCODING_PCM_16BIT, getPcmBufferSize() * 4);
        if (mic.getState() != AudioRecord.STATE_INITIALIZED) {
            mic = new AudioRecord(MediaRecorder.AudioSource.VOICE_COMMUNICATION, FasterEncoder.ASAMPLERATE,
                AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, getPcmBufferSize() * 4);
            if (mic.getState() != AudioRecord.STATE_INITIALIZED) {
                mic = null;
            } else {
                FasterEncoder.aChannelConfig = AudioFormat.CHANNEL_IN_MONO;
            }
        } else {
            FasterEncoder.aChannelConfig = AudioFormat.CHANNEL_IN_STEREO;
        }
