...
public class FasterPublisher {
    ...
    private FasterFlvMuxer mFlvMuxer;
    private FasterMp4Muxer mMp4Muxer;
    private FasterEncoder mEncoder;

    public FasterPublisher(FasterCameraView view) {
        mCameraView = view;
        mCameraView.setPreviewCallback(new FasterCameraView.PreviewCallback() {
            @Override
    ...
    private void calcSamplingFps() {
        ...
        if (videoFrameCount == 0) {
            ...
            if (++videoFrameCount >= FasterEncoder.VGOP) {
    ...
    public void setRtmpHandler(RtmpHandler handler) {
        mFlvMuxer = new FasterFlvMuxer(handler);
        if (mEncoder != null) {
            mEncoder.setFlvMuxer(mFlvMuxer);
        }
    }

    public void setRecordHandler(FasterRecordHandler handler) {
        mMp4Muxer = new FasterMp4Muxer(handler);
        if (mEncoder != null) {
            mEncoder.setMp4Muxer(mMp4Muxer);
        }
    }

    public void setEncodeHandler(FasterEncodeHandler handler) {
        mEncoder = new FasterEncoder(handler);
        if (mFlvMuxer != null) {
            mEncoder.setFlvMuxer(mFlvMuxer);
        }
        if (mMp4Muxer != null) {
